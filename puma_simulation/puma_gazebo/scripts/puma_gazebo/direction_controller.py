...
class DirectionController():
  ...
  def direction_to_angle(self):
    ...
    is_diff_angle_right = (self.current_angle*(1+self.tolerance) > self.angle_goal) and (current_angle_analog >= self.RIGHT_LIMIT_VALUE )
    is_diff_angle_left = (self.current_angle*(1-self.tolerance) < self.angle_goal) and (current_angle_analog <= self.LEFT_LIMIT_VALUE)
    
    #rospy.loginfo("Right angle limit: %s, left angle limit: %s", self.RIGHT_LIMIT_VALUE, self.LEFT_LIMIT_VALUE)
    #rospy.loginfo("Is diff angle to right: %s, is diff angle to left: %s", is_diff_angle_right, is_diff_angle_left)
    #rospy.loginfo("Current position: %s, current goal: %s", self.current_angle, self.angle_goal)
    
    if is_diff_angle_right:
      self.dir_value.data = self.dir_value.data + self.CONST_RAD_VALUE/3
      #rospy.loginfo("Increment direction to right")
    elif is_diff_angle_left:
      self.dir_value.data = self.dir_value.data - self.CONST_RAD_VALUE/3
    ...
    self.current_angle = self.dir_value.data*-1
