...
class PulseToVelocityConverter():
  ...
  '''
  def __init__(self):
    rospy.Subscriber('puma/sensors/tachometer', StatusTachometer, self._tachometer_callback)
    
    # Get params for calibrate
    ns = 'tachometer/'
    _calibrate_max_rpm = rospy.get_param(ns+'calibrate_max_rpm_motor', 1654) #
    _calibrate_max_velocity = rospy.get_param(ns+'calibrate_max_velocity', 22)  # Max velocity in mph
    self._wheels_diameter = rospy.get_param(ns+'wheels_diameter', 0.53) # in meters
    self._kalman_q_noise = rospy.get_param(ns+'kalman_q_noise', 7e-4)
    self._kalman_r_noise = rospy.get_param(ns+'kalman_r_noise', 0.001)
    # Init kalman filter
