...
class GoalFromPoseTag:
  ...
  def pose_from_tag(self, pose_received):
    ...
    goal.target_pose.pose.position.x = pose_received.pose.position.x - 2.5*math.cos(euler[2]+math.pi/2)
    goal.target_pose.pose.position.y = pose_received.pose.position.y - 2.5*math.sin(euler[2]+math.pi/2)
