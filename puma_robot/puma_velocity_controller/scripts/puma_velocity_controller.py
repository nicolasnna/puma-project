...
class PumaVelocityController():
  ...
  def _cmd_vel_callback(self, data_received):
    ...
    steering_angle, input_wheels = self.calculate_angles_velocities_output(linear_velocity, angular_velocity)
    rospy.loginfo(steering_angle)
    self.control_steering(steering_angle)
    self.control_wheels(input_wheels)
